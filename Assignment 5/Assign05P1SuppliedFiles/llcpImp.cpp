...
void PromoteTarget(Node* &headPtr, int target) {
	...
	if (headPtr->link == NULL && headPtr->data == target) {
		return;
	}
	Node* cursor = headPtr, * precursor = NULL;
	bool found = false;
	int count = 0;
	//while cursor true/not null/not 0
	while (cursor != 0) {

		if (cursor->data == target && count != 0) {
			if (cursor->link != NULL) {
				precursor->link = cursor->link;
				cursor->link = headPtr;
				headPtr = cursor;
		...
		else if (count == 0 && cursor->data == target) {
			precursor = cursor;
			cursor = cursor->link;
			if (cursor->link == NULL) { return; }
		}
		//if target not found and end of list, add value in new tail node
		else if (cursor->link == NULL && found == false) {
			Node* newNodePtr = new Node;
			newNodePtr->data = target;
			newNodePtr->link = 0;
			if (headPtr == 0) {
		...
		//if not end of list and current not target, traverse to next node
		else {
			precursor = cursor;
			cursor = cursor->link;
			count++;
