...
void SortedMergeRecur(Node*& xlist, Node*& ylist, Node*& zlist) {
	Node* xhead = xlist;
	Node* yhead = ylist;
	
	
	//these three if statements traverse if not at end of both lists, returns if both are
	if ((xlist == 0 || xlist->link == 0) && (ylist == 0 || ylist->link == 0)) {
		return;
	}
	...
	//direct recursion, continue traversing until ends of both found
	SortedMergeRecur(xlist, ylist, zlist);
	//once end of list has been found and returned, compare tail nodes and place largest at end of zlist, return
	xlist = xhead;
	ylist = yhead;
	if (xlist->data <= ylist->data) {
		...
		cout << xlist->data << endl;
		cout << ylist->data << endl;
		cout << zlist->data << endl;
		cout << "x is less" << endl;
		return;
	}
	if (ylist->data <= xlist->data) {
		xlist->link = zlist;
		zlist = xlist;
